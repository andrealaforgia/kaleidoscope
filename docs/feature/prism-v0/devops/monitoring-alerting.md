...
| KPI 5 violations (page-stays-usable in production) | Partial — KPI 5's CI fixture covers four named failure modes (PromQL parse, transport network failure, transport HTTP 500, empty result, config.json parse). Production may surface novel failure modes outside this set. **Operator mitigation REQUIRED at v0**: operators deploying Prism MUST enable their existing JS error-tracking tool (Sentry, Bugsnag, Rollbar, or Honeycomb's browser SDK) — uncaught browser exceptions surface there. Prism does not integrate at v0; document this as a deployment prerequisite in the v0 operator guide. CRITICAL-4 fix from Forge iter-1 review. | v0.x: a `prism.uncaught_error_count` gauge added to the emitter; trigger condition: after first production deployment surfaces a real KPI 5 incident |
