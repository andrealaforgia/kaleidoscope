...
- **Direct precedent**: `docs/feature/pulse-v1/devops/` — pulse v1 was
  the identical situation (a never-mutation-gated crate gaining a v1
  durable adapter). Ray v1 follows it exactly.

## Posture

Ray v1 inherits the five-gate workspace CI contract from ADR-0005.
Four of the five gates carry forward UNCHANGED. Gate 5 is the
exception: there is **no `gate-5-mutants-ray` job in `ci.yml` today**
(verified — `grep -c "gate-5-mutants-ray" .github/workflows/ci.yml`
returns 0; the existing Gate 5 jobs cover only aperture, codex,
harness, kaleidoscope-cli, pulse, self-observe, sieve, spark). Ray has
never been mutation-gated. Ray v1 is the moment to add it.
...
existing `gate-5-mutants-self-observe` job — exactly as pulse-v1 did
one day earlier (commit 4232c4a added `gate-5-mutants-pulse`).

## A1 — NEW `gate-5-mutants-ray` job (the one real change)

**Verdict: ADD a new per-package Gate 5 job, `gate-5-mutants-ray`.**

### Why a new job, not inheritance

The earlier v0-to-v1 carry-forwards in the platform plane (Cinder,
...
pure Gate 5 inheritance via the `--in-diff` cascade. Ray is different:
the `ray` crate has no Gate 5 job at all. The v0 walking skeleton
shipped before ray was wired into the mutation matrix. Pulse hit the
exact same gap one day ago and resolved it by adding
`gate-5-mutants-pulse` (A1 of `docs/feature/pulse-v1/devops/
wave-decisions.md`); ray repeats that move.

Adding `crates/ray/src/file_backed.rs` — the durable write and
...
honoured for ray without a job to run it. v1 is the correct moment:
the file that most needs mutation testing is the file this feature
introduces.

DD4 makes this sharper than the pulse case: the architecture rule
"ingest and recovery route through ONE `apply_ingest`, so the two
indices cannot drift" is verified structurally by there being exactly
...
no divergent twin". Without `gate-5-mutants-ray` there is no enforcer
for the central design invariant of the feature.
...
`gate-5-mutants-pulse` itself was mirrored from. It encodes the
current-best baseline cascade (`origin/main → HEAD~1 → full`), the
empty-diff short-circuit, the precompiled-binary install, and the
30-day artefact retention. Mirroring it (rather than the older
harness/aperture jobs) means ray inherits the latest conventions with
zero drift, and stays byte-identical to the pulse job added yesterday.
...
| # | Field | self-observe value | ray value |
|---|-------|--------------------|-----------|
| 1 | job key | `gate-5-mutants-self-observe` | `gate-5-mutants-ray` |
| 2 | step `name` | `Gate 5 — cargo mutants (self-observe)` | `Gate 5 — cargo mutants (ray)` |
| 3 | `--in-diff` path filter | `crates/self-observe/**` | `crates/ray/**` |
| 4 | `--package` arg | `--package self-observe` | `--package ray` |
| 5 | cache key suffix | `...-cargo-mutants-self-observe-...` | `...-cargo-mutants-ray-...` |
| 6 | artefact name | `mutants-out-self-observe` | `mutants-out-ray` |
...
`-self-observe` → `-ray` token. The diff-echo log lines and step
...
Per the constraint and per the Cinder/Lumen/Pulse precedent, this
DEVOPS wave does **NOT** edit `ci.yml`. `@nw-software-crafter` (Crafty)
lands the new `gate-5-mutants-ray` job atomic with the
...
`@nw-acceptance-designer` in DISTILL (from US-RV1-01, US-RV1-02) run
...
## A3 — `serde` + `serde_json` enter ray's `[dependencies]` (NO new external crate)
...
accurate for the `ray` crate. Verified by reading
`crates/ray/Cargo.toml`: ray v0's `[dependencies]` block contains
**only** `aegis`. `serde` and `serde_json` are NOT direct dependencies
of ray v0.

This is the same premise error the pulse-v1 DEVOPS wave caught (A3 of
`docs/feature/pulse-v1/devops/wave-decisions.md`): ray, like pulse,
pulls serde transitively via aegis today but does not declare it.
...
`aegis` and the other durable crates. So ray v1's serde derives
(DD2/DD5/D5), including the custom `Serialize`/`Deserialize` impls on
`TraceId`/`SpanId` and the hand-rolled hex codec, require adding to
ray's OWN `[dependencies]`:
...
This is a new entry in `crates/ray/Cargo.toml`, but it adds **zero new
external crates to the workspace dependency graph** — both are already
resolved in `Cargo.lock`. The hex codec being hand-rolled (DD5) means
no `serde_with` and no `hex` crate enters the graph either. The
distinction matters for Gate 4:
...
`serde`/`serde_json` and a ~20-line hand-rolled hex module; no MSRV
bump (memory `feedback_msrv_creep_is_ecosystem_reality` does not
trigger — no transitive dep raises its `rust-version`), no nightly
feature, no new component. The new `gate-5-mutants-ray` job uses the
...
| Gate 2 (`cargo public-api`) | UNCHANGED | ray not in the Gate 2 scope set {harness, spark, sieve, codex}; not graduated by this feature |
...
| Gate 5 (`cargo mutants`) | **NEW JOB** | `gate-5-mutants-ray` added (A1) |
...
| `scripts/hooks/pre-push` | None. The per-pkg loop for Gates 2/3 iterates `[otlp-conformance-harness, spark, sieve, codex]`; ray is not graduated to Gates 2/3 by this feature, so it is not added. |
...
- **Lead time**: commit to available-to-downstream = time-to-merge;
  ...
  `gate-5-mutants-ray` job runs in parallel with the other Gate 5 jobs
  (independent `needs`), so it does not lengthen the critical path
  beyond the existing slowest Gate 5 job.
- **Change failure rate**: failed Gate 1 or Gate 5 over the next 10
  ray-touching commits. Target 0%. The new Gate 5 job makes ray
...
substrate across BOTH the trace index and the service index, including
the empty-`service.name` edge case. The new `gate-5-mutants-ray` job
is the test-quality probe that proves those gold-tests can distinguish
the correct adapter from a behaviourally-mutated one — and is the only
mechanism that kills a divergent second copy of `apply_ingest` (DD4).
`BufWriter::flush` is not fsync; `kill -9`-between-flush-and-fsync
recovery is documented v2 scope, not a v1 gate.
