...
|-----|------|--------|--------------------|------|----------------------------------|
| KPI1 — ingest latency | leading | p95 <= 2 ms, 1 000 trials, debug, 100-span batch | `ray::tests::v1_slice_01_wal_durability::ingest_p95_latency_under_two_milliseconds` | Gate 1 | YES — test name + method present in outcome-kpis.md §KPI1 |
...
- **Leading indicators**: KPI1 ingest latency and KPI2 recovery time —
  ...
  buckets), but per outcome-kpis.md the durable costs dominate and the
  2 ms / 2.5 s budgets hold.
...
KPI1 (2 ms) and KPI2 (2.5 s) adopt the post-bump Pulse v1 / Cinder v1
/ Lumen v1 figures up front. The "alert" (a red Gate 1) therefore
fires on a genuine regression, not on substrate noise.
