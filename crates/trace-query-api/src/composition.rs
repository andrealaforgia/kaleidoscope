...
/// `KALEIDOSCOPE_TRACE_QUERY_AUTH_*` keys; all four must be present
/// together or wholly absent (a partial set refuses to start).
pub const AUTH_ISSUER_ENV: &str = "KALEIDOSCOPE_TRACE_QUERY_AUTH_ISSUER";
/// Env var carrying the read-auth audience (ADR-0074 DD1/DD6;
/// `kaleidoscope-query` is the read audience).
pub const AUTH_AUDIENCE_ENV: &str = "KALEIDOSCOPE_TRACE_QUERY_AUTH_AUDIENCE";
...
pub const AUTH_SECRET_FILE_ENV: &str = "KALEIDOSCOPE_TRACE_QUERY_AUTH_SECRET_FILE";
/// Env var carrying the PATH to the aegis tenant catalogue (ADR-0074 DD1).
pub const AUTH_CATALOGUE_ENV: &str = "KALEIDOSCOPE_TRACE_QUERY_AUTH_CATALOGUE";
...
const STARTUP_PROBE_TENANT: &str = "__trace_query_api_startup_probe__";
...
///
/// The four `KALEIDOSCOPE_TRACE_QUERY_AUTH_*` env values are passed in raw
...
/// naming every missing `KALEIDOSCOPE_TRACE_QUERY_AUTH_*` key (never a
/// secret).
fn partial_reason(present: &[bool; 4]) -> String {
    ...
    let missing: Vec<&str> = names
        ...
        .collect();
    format!(
        "partial read-auth config: missing {} (all four KALEIDOSCOPE_TRACE_QUERY_AUTH_* keys are required when read-auth is enabled)",
        missing.join(", ")
    )
}
...
mod tests {
    ...
    fn unique(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!("kaleidoscope-tqa-authcfg-unit-{label}-{stamp}"))
    }
