// Kaleidoscope trace-query-api — slice 06 read-auth composition-root config reject
...
//! Slice 06 — read-auth composition-root config wiring (the closers).
//!
...
//! `slice_08_auth_config_reject.rs` refuse-to-start precedent on the traces
//! read binary.
//!
//! ## The matrix under test (DD1/DD4)
//!
//! | `KALEIDOSCOPE_TRACE_QUERY_AUTH_*` | Result | Event | Exit | Listener |
//! |---|---|---|---|---|
...
//!
//! The real `trace-query-api` binary subprocess, observed through its exit
//! code and structured stderr. EPHEMERAL ports only: every spawn sets
//! `KALEIDOSCOPE_TRACE_QUERY_ADDR=127.0.0.1:0`, so the fixed `9092` default is
...
fn trace_query_api_bin() -> Command {
    Command::new(env!("CARGO_BIN_EXE_trace-query-api"))
}
...
fn unique_path(label: &str, ext: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("kaleidoscope-tqa-authcfg-{label}-{stamp}.{ext}"))
}
...
fn partial_auth_config_refuses_to_start_naming_the_missing_key() {
    ...
    let (code, stderr) = run_bounded(
        {
            let mut cmd = trace_query_api_bin();
            cmd.env("KALEIDOSCOPE_PILLAR_ROOT", &pillar)
                .env("KALEIDOSCOPE_TRACE_QUERY_TENANT", "acme-prod")
                .env("KALEIDOSCOPE_TRACE_QUERY_ADDR", "127.0.0.1:0")
                .env("KALEIDOSCOPE_TRACE_QUERY_AUTH_ISSUER", "acme-observability")
                .env(
                    "KALEIDOSCOPE_TRACE_QUERY_AUTH_AUDIENCE",
                    "kaleidoscope-query",
                )
                .env_remove("KALEIDOSCOPE_TRACE_QUERY_AUTH_SECRET_FILE")
                .env_remove("KALEIDOSCOPE_TRACE_QUERY_AUTH_CATALOGUE");
            cmd
        },
    ...
    let (code, stderr) = run_bounded(
        {
            ...
            cmd.env("KALEIDOSCOPE_PILLAR_ROOT", &pillar)
                ...
                .env("KALEIDOSCOPE_TRACE_QUERY_AUTH_SECRET_FILE", &missing_secret)
                .env("KALEIDOSCOPE_TRACE_QUERY_AUTH_CATALOGUE", &catalogue);
            cmd
        },
        Duration::from_secs(8),
        None,
    ...
    let (_, stderr) = run_bounded(
        {
            let mut cmd = trace_query_api_bin();
            cmd.env("KALEIDOSCOPE_PILLAR_ROOT", &pillar)
                .env_remove("KALEIDOSCOPE_TRACE_QUERY_TENANT")
                .env("KALEIDOSCOPE_TRACE_QUERY_ADDR", "127.0.0.1:0")
                .env("KALEIDOSCOPE_TRACE_QUERY_AUTH_ISSUER", "acme-observability")
                .env(
                    "KALEIDOSCOPE_TRACE_QUERY_AUTH_AUDIENCE",
                ...
                .env("KALEIDOSCOPE_TRACE_QUERY_AUTH_SECRET_FILE", &secret)
                .env("KALEIDOSCOPE_TRACE_QUERY_AUTH_CATALOGUE", &catalogue);
            cmd
        },
    ...
    let (_, stderr) = run_bounded(
        {
            ...
            cmd.env("KALEIDOSCOPE_PILLAR_ROOT", &pillar)
                .env("KALEIDOSCOPE_TRACE_QUERY_TENANT", "acme-prod")
                .env("KALEIDOSCOPE_TRACE_QUERY_ADDR", "127.0.0.1:0")
                .env_remove("KALEIDOSCOPE_TRACE_QUERY_AUTH_ISSUER")
                .env_remove("KALEIDOSCOPE_TRACE_QUERY_AUTH_AUDIENCE")
                .env_remove("KALEIDOSCOPE_TRACE_QUERY_AUTH_SECRET_FILE")
                .env_remove("KALEIDOSCOPE_TRACE_QUERY_AUTH_CATALOGUE");
            cmd
        },
