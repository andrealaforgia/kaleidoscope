// Kaleidoscope query-api — slice 08 read-auth composition-root config reject
...
//! Slice 08 — read-auth composition-root config wiring (the closers).
//!
//! Feature: `read-path-query-api-auth-v0` (ADR-0074 DD1/DD4). This is the
//! 6th load-bearing security control DISTILL deferred to DELIVER: it lives
//! at the `main`/composition-root boundary, NOT the `router()` seam the
//! in-process auth suites drive. It mirrors aperture's
//! `slice_10_ingest_auth_config_reject.rs` refuse-to-start precedent on the
//! metrics read binary.
//!
//! ## The matrix under test (DD1/DD4)
//!
//! | `KALEIDOSCOPE_QUERY_AUTH_*` | Result | Event | Exit | Listener |
//! |---|---|---|---|---|
...
//!
//! The real `query-api` binary subprocess, observed through its exit code
//! and structured stderr. EPHEMERAL ports only: every spawn sets
//! `KALEIDOSCOPE_QUERY_ADDR=127.0.0.1:0`, so the fixed `9090` default is
//! never bound (no fixed-port flake; cf. the aperture-4317 project memory).
//! The "no listener" observable on a refusal is the non-zero exit BEFORE the
//! bind step — the binary is gone, nothing is serving.
//!
//! ## RED-not-BROKEN classification (Mandate 7)
//!
//! The binary builds today and these tests COMPILE + spawn it. They are
//! behaviourally RED because today's `main` IGNORES the four read-auth env
//! vars: a partial / unreadable config STARTS and binds (no refusal), so the
//! exit-non-zero + `config_validation_failed` assertions fail; and a complete
//! config with the env tenant UNSET fail-closes on the tenant probe, so the
//! "starts and binds" assertion fails. They pass once DELIVER wires
//! `resolve_read_auth` + the synthetic startup-probe tenant into `main`.
...
fn query_api_bin() -> Command {
    Command::new(env!("CARGO_BIN_EXE_query-api"))
}
...
fn unique_path(label: &str, ext: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("kaleidoscope-qa-authcfg-{label}-{stamp}.{ext}"))
}
...
///
/// FALSIFIABILITY: today's binary ignores the auth vars, the tenant resolves,
/// the store is readable, so it BINDS and runs forever (exit None) — the
/// `Some(non-zero)` + event assertions fail. Green only once `resolve_read_auth`
/// refuses a partial config in `main`.
#[test]
fn partial_auth_config_refuses_to_start_naming_the_missing_key() {
    ...
    let (code, stderr) = run_bounded(
        {
            let mut cmd = query_api_bin();
            cmd.env("KALEIDOSCOPE_PILLAR_ROOT", &pillar)
                .env("KALEIDOSCOPE_QUERY_TENANT", "acme-prod")
                .env("KALEIDOSCOPE_QUERY_ADDR", "127.0.0.1:0")
                .env("KALEIDOSCOPE_QUERY_AUTH_ISSUER", "acme-observability")
                .env("KALEIDOSCOPE_QUERY_AUTH_AUDIENCE", "kaleidoscope-query")
                .env_remove("KALEIDOSCOPE_QUERY_AUTH_SECRET_FILE")
                .env_remove("KALEIDOSCOPE_QUERY_AUTH_CATALOGUE");
            cmd
        },
...
///
/// FALSIFIABILITY: today the auth vars are ignored, so the binary binds (exit
/// None); the exit-2 + event assertions fail. Green only once the readability
/// check in `resolve_read_auth` refuses.
#[test]
    ...
    let (code, stderr) = run_bounded(
        {
            ...
            cmd.env("KALEIDOSCOPE_PILLAR_ROOT", &pillar)
                ...
                .env("KALEIDOSCOPE_QUERY_AUTH_ISSUER", "acme-observability")
                .env("KALEIDOSCOPE_QUERY_AUTH_AUDIENCE", "kaleidoscope-query")
                .env("KALEIDOSCOPE_QUERY_AUTH_SECRET_FILE", &missing_secret)
                .env("KALEIDOSCOPE_QUERY_AUTH_CATALOGUE", &catalogue);
            cmd
        },
        Duration::from_secs(8),
        None,
...
/// tenant. The auth startup negative probe (DD4) passes (a known-bad token is
/// rejected), so neither refusal event appears.
///
/// FALSIFIABILITY: today the env tenant is unset, so the binary fail-closes on
/// the tenant probe (`health.startup.refused`) and never binds — the
/// `listener_bound` assertion fails. Green only once auth-on uses a synthetic
/// startup-probe tenant so the store readability check passes without an env
/// tenant.
#[test]
    ...
    let (_, stderr) = run_bounded(
        {
            let mut cmd = query_api_bin();
            cmd.env("KALEIDOSCOPE_PILLAR_ROOT", &pillar)
                .env_remove("KALEIDOSCOPE_QUERY_TENANT")
                .env("KALEIDOSCOPE_QUERY_ADDR", "127.0.0.1:0")
                .env("KALEIDOSCOPE_QUERY_AUTH_ISSUER", "acme-observability")
                .env("KALEIDOSCOPE_QUERY_AUTH_AUDIENCE", "kaleidoscope-query")
                .env("KALEIDOSCOPE_QUERY_AUTH_SECRET_FILE", &secret)
                .env("KALEIDOSCOPE_QUERY_AUTH_CATALOGUE", &catalogue);
            cmd
        },
    ...
    let (_, stderr) = run_bounded(
        {
            ...
            cmd.env("KALEIDOSCOPE_PILLAR_ROOT", &pillar)
                .env("KALEIDOSCOPE_QUERY_TENANT", "acme-prod")
                .env("KALEIDOSCOPE_QUERY_ADDR", "127.0.0.1:0")
                .env_remove("KALEIDOSCOPE_QUERY_AUTH_ISSUER")
                .env_remove("KALEIDOSCOPE_QUERY_AUTH_AUDIENCE")
                .env_remove("KALEIDOSCOPE_QUERY_AUTH_SECRET_FILE")
                .env_remove("KALEIDOSCOPE_QUERY_AUTH_CATALOGUE");
            cmd
        },
