...
use query_api::composition::{
    probe, resolve_addr, resolve_pillar_root, resolve_read_auth, resolve_static_dir,
    resolve_tenant, startup_probe_tenant, PULSE_SUBDIR,
};
...
async fn main() -> ExitCode {
    ...
    // every lifecycle event from `query_api_starting` onward (and any
    ...
    let auth = match resolve_read_auth(
        std::env::var("KALEIDOSCOPE_QUERY_AUTH_ISSUER").ok(),
        std::env::var("KALEIDOSCOPE_QUERY_AUTH_AUDIENCE").ok(),
        std::env::var("KALEIDOSCOPE_QUERY_AUTH_SECRET_FILE").ok(),
        std::env::var("KALEIDOSCOPE_QUERY_AUTH_CATALOGUE").ok(),
    ) {
        Ok(auth) => auth,
        Err(reason) => {
    ...
    let store: Arc<dyn MetricStore + Send + Sync> =
        match FileBackedMetricStore::open(&pulse_path, Box::new(NoopRecorder)) {
    ...
    let addr = match resolve_addr(std::env::var("KALEIDOSCOPE_QUERY_ADDR").ok()) {
        Ok(addr) => addr,
        Err(reason) => {
            tracing::error!(event = "health.startup.refused", reason = %reason);
            return ExitCode::from(EXIT_STARTUP_ERROR);
    ...
    let app = query_api::router_with_auth(store, tenant, auth, static_dir);
